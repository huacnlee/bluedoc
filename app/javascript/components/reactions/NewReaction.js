...
export default class NewReaction extends React.Component {
  ...
  render() {
    ...
    return <details class="details-overlay details-reset position-relative float-left reaction-popover-container" ref={this.menuRef}>
      <summary class="reaction-summary-item add-reaction-btn"><Icon name="smile" /></summary>
      <div class="dropdown-menu dropdown-menu-ne add-reaction-popover anim-scale-in">
        ...
        <div class="add-reactions-options mx-1 mb-3">
          {allowReactions.map(reaction => (
            <Reaction {...this.props} reaction={reaction} onSelect={this.onSelect} className="add-reactions-options-item" />
          ))}
        </div>
      </div>
    </details>
